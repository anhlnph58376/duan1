...
class Guide extends BaseModel
{
    ...
    function getGuideByUserId($user_id)
    {
    ...
    function deleteGuideByUserId($user_id)
    {
        $sql = "DELETE FROM tour_guides WHERE user_id = :user_id";
        $stmt = $this->pdo->prepare($sql);
    ...
    // Get tours assigned to a guide
    public function getGuideAssignedTours($guide_id)
    {
        try {
            // Get bookings directly assigned to this guide
            $sql = "SELECT b.id as booking_id, b.booking_code, b.booking_date, b.status as booking_status,
                           t.id as tour_id, t.name as tour_name, t.start_date, t.end_date,
    ...
    public function getAvailableGuides($date, $guide_type = null)
    {
        $sql = "SELECT g.* FROM tour_guides g
                ...
                AND u.role_id = 2"; // Only users with Tour Guide role (role_id = 2)
    ...
    public function searchGuides($search_term)
    {
        $sql = "SELECT * FROM tour_guides
                WHERE (name LIKE :search_term OR phone LIKE :search_term OR email LIKE :search_term)
                AND status = 'Active'";
        $stmt = $this->pdo->prepare($sql);
    ...
    // Add setLastError method since BaseModel doesn't have it
    protected function setLastError($msg)
    {
        // Simple implementation - could be enhanced to log to file like Booking model
        error_log("Guide Error: " . $msg);
    }
}