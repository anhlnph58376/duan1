                                ...
                                <tbody>
                                    ...
                                    if ($guideId) {
                                        ...
                                        $stmt = $pdo->prepare("
                                            SELECT d.id, CONCAT('DEP-', d.id) as booking_code, d.departure_date as start_date, 
                                                   d.status, d.max_pax as number_of_people, tv.tour_id, '' as tour_title, 
                                                   '' as customer_name
                                            FROM departures d
                                            LEFT JOIN tour_versions tv ON tv.id = d.tour_version_id
                                            WHERE d.actual_guide_id = :guide_id
                                    ...
                                    <?php if (!empty($bookings)): foreach ($bookings as $booking): ?>
                                        <tr>
                                            ...
                                            <td><?= (int)($booking['number_of_people'] ?? 0) ?></td>
                                    ...
                                    <?php endforeach; else: ?>
                                        <tr><td colspan="7" class="text-center text-muted">Chưa có booking nào được gán</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
