...
class AccountController
{
    ...
    public function store()
    {
        ...
        // If this is a Tour Guide (role_id = 2), automatically create a guide record
        if ($role_id == 2) {
            $guideModel = new Guide();
            $guideModel->addGuide($userId, $username, '', '', null, null, 'Active');
        }
    ...
    public function update()
    {
        ...
        // Check if this user is currently a guide
        $currentUser = $userModel->find($id);
        $isCurrentlyGuide = $guideModel->getGuideByUserId($id) !== false;
        ...
        // Handle guide creation/removal based on role changes
        if ($role_id == 2) { // Tour Guide role
            if (!$isCurrentlyGuide) {
                // User is now a guide but wasn't before - create guide record
                $guideModel->addGuide($id, $username, '', '', null, null, 'Active');
            }
        } else {
            if ($isCurrentlyGuide) {
                // User is no longer a guide - remove guide record
                $guideModel->deleteGuideByUserId($id);
            }
        }
